...
if ($edit!=null && $_SERVER["REQUEST_METHOD"]=="POST") {
    ...
    $sql = "UPDATE youtube SET title='$title_form',description='$desc_form' WHERE id='$edit' AND creator='$username'";
    if ($link->query($sql) === TRUE) {} else {$msg="<div class='alert alert-danger'>Dir gehört dieses Video nicht oder es gibt es nicht!</div>";}
}

// Delete Video
$delete=$_GET["del"] ?? null;
if ($delete!=null) {
    $sql = "DELETE FROM youtube WHERE id='$delete' AND creator='$username'";
    if ($link->query($sql) === TRUE) {
        unlink('./videos/'.$delete.'.mp4');
        unlink('./videos/'.$delete.'.png');
        header("Location: ./studio");
    } else $msg="<div class='alert alert-danger'>Dir gehört dieses Video nicht oder es gibt es nicht!</div>";
}
?>
<!DOCTYPE html>
<html lang="de">
<head>
    <title>Studio von <?php echo $username?> | DataTube</title>
    <meta charset="UTF-8">
    ...
    <div class="d-flex" id="wrapper">
        <div id="sidebar" class="bg-body-tertiary">
            ...
            <nav class="list-group list-group-flush px-3">
                ...
                <a href="./studio" class="btn btn-primary text-start fs-5 mb-2"><i class="fa-solid fa-pen-to-square m"></i> Studio</a>
                <hr>
                <h5>Andere Kanäle:</h5>
                <?php
        ...
        <div id="content" class="w-100">
            ...
            <div class="p-2">
                <table id="studiovideostable" class="table">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                    <?php
                    $result = mysqli_query($link, "SELECT * FROM youtube WHERE creator='$username'");
                    if (mysqli_num_rows($result)>0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            $unixtime=$row["date"];
                            $datef=date('j. M Y H:i', $unixtime);
