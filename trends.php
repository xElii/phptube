    ...
    <div class="d-flex" id="wrapper">
        <div id="sidebar" class="bg-body-tertiary">
            ...
            <nav class="list-group list-group-flush px-3">
                ...
                <a href="./trends" class="btn btn-primary text-start fs-5"><i class="fa-solid fa-fire m"></i> Trends</a>
                <hr>
                <h5>Dein Kanal:</h5>
        ...
        <div id="content" class="w-100">
            ...
            <h1 class="pt-4 ps-4">Top 10 Most Likes Videos:</h1>
            <div class="row p-4 row-cols-3 col-lg-12" id="videos_row">
                <?php
                $result = mysqli_query($link, "SELECT * FROM youtube ORDER BY likes DESC LIMIT 10");
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        $unixtime=$row["date"];
                        $datef=date('j. M Y H:i', $unixtime);
                        ...
                        <a href="./watch?v='.$row["id"].'" class="col mb-3 videocard" title="'.$row["title"].'">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <p class="d-flex align-content-center my-1"><img src="//datalok.de/account/api?type=pf&u='.$row["creator"].'" width="25" class="rounded-5 m"> '.$row["creator"].' • '.$datef.' • '.$row["views"].' Aufrufe</p>
                                </div>
                            </div>
                        </a>
                        ';
